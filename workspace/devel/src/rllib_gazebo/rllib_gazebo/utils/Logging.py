...
from rllib_gazebo.utils.ArgsConfig import EntryArgs
...
class Logger():
    ...
    def initialize_once(cls):
        ...
        cls.logging_mode = EntryArgs().logging_mode
        cls.console_level = cls.convert(EntryArgs().console_level)
        cls.file_level = cls.convert(EntryArgs().file_level)
        ...
        if cls.console_level is not None:
            ...
            cls.console_logger = logging.Logger(f'console_logger ({EntryArgs().ENTRY})')
        ...
        if cls.file_level is not None:
            log_path = os.path.join(Exchanger().path_register['log'], f'{EntryArgs().ENTRY}.log')
            # create file handler and formatter
            file_handler = logging.FileHandler(log_path)
            file_formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
            file_handler.setFormatter(file_formatter)
            # create file logger and add handler
            cls.file_logger = logging.Logger(f'file_logger ({EntryArgs().ENTRY})')
