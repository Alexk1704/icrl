...
void createTestMsg(gz::msgs::Physics & _msg)
{
  createTestMsg(*_msg.mutable_header());

  _msg.set_type(gz::msgs::Physics::DART);
  _msg.set_solver_type("test_solver_type");
  _msg.set_min_step_size(0.001);
  _msg.set_precon_iters(100);
  _msg.set_iters(200);
  _msg.set_sor(0.002);
  _msg.set_cfm(0.003);
  _msg.set_erp(0.004);
  _msg.set_contact_max_correcting_vel(0.005);
  _msg.set_contact_surface_layer(0.006);
  createTestMsg(*_msg.mutable_gravity());
  _msg.set_enable_physics(true);
  _msg.set_real_time_factor(0.007);
  _msg.set_real_time_update_rate(0.008);
  _msg.set_max_step_size(0.009);
  _msg.set_profile_name("test_profile_name");
  createTestMsg(*_msg.mutable_magnetic_field());
}

void compareTestMsg(const std::shared_ptr<gz::msgs::Physics> & _msg)
{
  gz::msgs::Physics expected_msg;
  createTestMsg(expected_msg);
  compareTestMsg(std::make_shared<gz::msgs::Header>(_msg->header()));

  EXPECT_EQ(expected_msg.type(), _msg->type());
  EXPECT_EQ(expected_msg.solver_type(), _msg->solver_type());
  EXPECT_EQ(expected_msg.min_step_size(), _msg->min_step_size());
  EXPECT_EQ(expected_msg.precon_iters(), _msg->precon_iters());
  EXPECT_EQ(expected_msg.iters(), _msg->iters());
  EXPECT_EQ(expected_msg.sor(), _msg->sor());
  EXPECT_EQ(expected_msg.cfm(), _msg->cfm());
  EXPECT_EQ(expected_msg.erp(), _msg->erp());
  EXPECT_EQ(expected_msg.contact_max_correcting_vel(), _msg->contact_max_correcting_vel());
  EXPECT_EQ(expected_msg.contact_surface_layer(), _msg->contact_surface_layer());
  compareTestMsg(std::make_shared<gz::msgs::Vector3d>(_msg->gravity()));
  EXPECT_EQ(expected_msg.enable_physics(), _msg->enable_physics());
  EXPECT_EQ(expected_msg.real_time_factor(), _msg->real_time_factor());
  EXPECT_EQ(expected_msg.real_time_update_rate(), _msg->real_time_update_rate());
  EXPECT_EQ(expected_msg.max_step_size(), _msg->max_step_size());
  EXPECT_EQ(expected_msg.profile_name(), _msg->profile_name());
  compareTestMsg(std::make_shared<gz::msgs::Vector3d>(_msg->magnetic_field()));
}
...
{
  _msg.set_name("entity");
  _msg.set_allow_renaming(true);
  _msg.set_sdf("");
  _msg.set_sdf_filename("test_sdf_filename");
  _msg.set_clone_name("test_clone_name");
  createTestMsg(*_msg.mutable_pose());
  _msg.set_relative_to("world");
}
...
void compareTestMsg(const std::shared_ptr<gz::msgs::EntityFactory> & _msg)
{
  ...
  createTestMsg(expected_msg);
  EXPECT_EQ(expected_msg.name(), _msg->name());
  EXPECT_EQ(expected_msg.allow_renaming(), _msg->allow_renaming());
  EXPECT_EQ(expected_msg.sdf(), _msg->sdf());
  EXPECT_EQ(expected_msg.sdf_filename(), _msg->sdf_filename());
  EXPECT_EQ(expected_msg.clone_name(), _msg->clone_name());
  compareTestMsg(std::make_shared<gz::msgs::Pose>(_msg->pose()));
  EXPECT_EQ(expected_msg.relative_to(), _msg->relative_to());
}
