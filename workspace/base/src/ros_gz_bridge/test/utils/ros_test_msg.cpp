...
void createTestMsg(ros_gz_interfaces::msg::Physics & _msg)
{
  createTestMsg(_msg.header);

  _msg.type = ros_gz_interfaces::msg::Physics::DART;
  _msg.solver_type = "test_solver_type";
  _msg.min_step_size = 0.001;
  _msg.precon_iters = 100;
  _msg.iters = 200;
  _msg.sor = 0.002;
  _msg.cfm = 0.003;
  _msg.erp = 0.004;
  _msg.contact_max_correcting_vel = 0.005;
  _msg.contact_surface_layer = 0.006;
  createTestMsg(_msg.gravity);
  _msg.enable_physics = true;
  _msg.real_time_factor = 0.007;
  _msg.real_time_update_rate = 0.008;
  _msg.max_step_size = 0.009;
  _msg.profile_name = "test_profile_name";
  createTestMsg(_msg.magnetic_field);
}

void compareTestMsg(const std::shared_ptr<ros_gz_interfaces::msg::Physics> & _msg)
{
  ros_gz_interfaces::msg::Physics expected_msg;
  createTestMsg(expected_msg);
  compareTestMsg(_msg->header);
  ...
  compareTestMsg(std::make_shared<geometry_msgs::msg::Vector3>(_msg->gravity));
...
{
  _msg.name = "entity";
  _msg.allow_renaming = true;
  _msg.sdf = "";
  _msg.sdf_filename = "test_sdf_filename";
  _msg.clone_name = "test_clone_name";
  createTestMsg(_msg.pose);
  _msg.relative_to = "world";
}
