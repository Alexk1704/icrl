  ...
  ros_gz_interfaces::msg::Entity & ros_msg)
{
  ...
  if (gz_msg.type() == gz::msgs::Entity::NONE) {
...
{
  convert_ros_to_gz(ros_msg.header, (*gz_msg.mutable_header()));
  convert_ros_to_gz(ros_msg.system, (*gz_msg.mutable_system()));
  convert_ros_to_gz(ros_msg.real, (*gz_msg.mutable_real()));
  convert_ros_to_gz(ros_msg.sim, (*gz_msg.mutable_sim()));
}
...
{
  convert_gz_to_ros(gz_msg.header(), ros_msg.header);
  convert_gz_to_ros(gz_msg.system(), ros_msg.system);
  convert_gz_to_ros(gz_msg.real(), ros_msg.real);
  convert_gz_to_ros(gz_msg.sim(), ros_msg.sim);
}
...
void
convert_ros_to_gz(
  const ros_gz_interfaces::msg::Physics & ros_msg,
  gz::msgs::Physics & gz_msg)
{
  convert_ros_to_gz(ros_msg.header, (*gz_msg.mutable_header()));

  switch (ros_msg.type) {
    case ros_gz_interfaces::msg::Physics::ODE:
      gz_msg.set_type(gz::msgs::Physics::ODE);
      break;
    case ros_gz_interfaces::msg::Physics::BULLET:
      gz_msg.set_type(gz::msgs::Physics::BULLET);
      break;
    case ros_gz_interfaces::msg::Physics::SIMBODY:
      gz_msg.set_type(gz::msgs::Physics::SIMBODY);
      break;
    case ros_gz_interfaces::msg::Physics::DART:
      gz_msg.set_type(gz::msgs::Physics::DART);
      break;
    default:
      std::cerr << "Unsupported physics type [" << ros_msg.type << "]\n";
  }

  gz_msg.set_solver_type(ros_msg.solver_type);
  gz_msg.set_min_step_size(ros_msg.min_step_size);
  gz_msg.set_precon_iters(ros_msg.precon_iters);
  gz_msg.set_iters(ros_msg.iters);
  gz_msg.set_sor(ros_msg.sor);
  gz_msg.set_cfm(ros_msg.cfm);
  gz_msg.set_erp(ros_msg.erp);
  gz_msg.set_contact_max_correcting_vel(ros_msg.contact_max_correcting_vel);
  gz_msg.set_contact_surface_layer(ros_msg.contact_surface_layer);
  convert_ros_to_gz(ros_msg.gravity, (*gz_msg.mutable_gravity()));
  gz_msg.set_enable_physics(ros_msg.enable_physics);
  gz_msg.set_real_time_factor(ros_msg.real_time_factor);
  gz_msg.set_real_time_update_rate(ros_msg.real_time_update_rate);
  gz_msg.set_max_step_size(ros_msg.max_step_size);
  gz_msg.set_profile_name(ros_msg.profile_name);
  convert_ros_to_gz(ros_msg.magnetic_field, (*gz_msg.mutable_magnetic_field()));
}

template<>
void
convert_gz_to_ros(
  const gz::msgs::Physics & gz_msg,
  ros_gz_interfaces::msg::Physics & ros_msg)
{
  convert_gz_to_ros(gz_msg.header(), ros_msg.header);

  if (gz_msg.type() == gz::msgs::Physics::ODE) {
    ros_msg.type = ros_gz_interfaces::msg::Physics::ODE;
  } else if (gz_msg.type() == gz::msgs::Physics::BULLET) {
    ros_msg.type = ros_gz_interfaces::msg::Physics::BULLET;
  } else if (gz_msg.type() == gz::msgs::Physics::SIMBODY) {
    ros_msg.type = ros_gz_interfaces::msg::Physics::SIMBODY;
  } else if (gz_msg.type() == gz::msgs::Physics::DART) {
    ros_msg.type = ros_gz_interfaces::msg::Physics::DART;
  } else {
    std::cerr << "Unsupported Physics [" <<
      gz_msg.type() << "]" << std::endl;
  }

  ros_msg.type = gz_msg.type();
  ros_msg.solver_type = gz_msg.solver_type();
  ros_msg.min_step_size = gz_msg.min_step_size();
  ros_msg.precon_iters = gz_msg.precon_iters();
  ros_msg.iters = gz_msg.iters();
  ros_msg.sor = gz_msg.sor();
  ros_msg.cfm = gz_msg.cfm();
  ros_msg.erp = gz_msg.erp();
  ros_msg.contact_max_correcting_vel = gz_msg.contact_max_correcting_vel();
  ros_msg.contact_surface_layer = gz_msg.contact_surface_layer();
  convert_gz_to_ros(gz_msg.gravity(), ros_msg.gravity);
  ros_msg.enable_physics = gz_msg.enable_physics();
  ros_msg.real_time_factor = gz_msg.real_time_factor();
  ros_msg.real_time_update_rate = gz_msg.real_time_update_rate();
  ros_msg.max_step_size = gz_msg.max_step_size();
  ros_msg.profile_name = gz_msg.profile_name();
  convert_gz_to_ros(gz_msg.magnetic_field(), ros_msg.magnetic_field);
}
